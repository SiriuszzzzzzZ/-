...
export function MoodChart({ trend, showChart }: { trend: MoodTrend[]; showChart: boolean }) {
  if (!showChart || trend.length === 0) {
    return (
      <Card className="text-center py-4 bg-white/60">
        <p className="text-sm text-warm-400">今日数据不足，暂不展示趋势</p>
        <p className="text-xs text-warm-300 mt-1">至少需要 5 人参与</p>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <h3 className="text-sm font-medium text-warm-600 mb-3">本周情绪趋势</h3>
      <div className="flex items-end gap-1 h-20">
        {trend.map((d) => (
          <div key={d.date} className="flex-1 flex flex-col items-center">
            ...
            <span className="text-[10px] text-warm-300 mt-1">{d.date.slice(5)}</span>
          </div>
        ))}
      </div>
    </Card>
